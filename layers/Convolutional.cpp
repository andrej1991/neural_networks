...
Matrix** Convolutional::derivate_layers_output(Matrix **input, int threadindex){
    this->feedforward_helpter->helper[threadindex]->zero();
    for(int map_index = 0; map_index < this->map_count; map_index++){   ///allready calculated, might happen that in RNN case it's going to be needed again to be calculated
