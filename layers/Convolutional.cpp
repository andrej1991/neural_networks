...
Matrix** Convolutional::backpropagate(Matrix **input, Layer *next_layer, Feature_map** nabla, Matrix **delta, int threadindex)
{
    ...
    else if (this->next_layers_type == CONVOLUTIONAL)
    {
        ...
        for(int i = 0; i < this->map_count; i++)
        {
            ...
            for(int j = 0; j < next_layers_fmapcount; j++)
            {
                calculate_delta_helper(this->backprop_helper->padded_delta[j][0], this->layers_delta_helper[threadindex][i],
...
//        delete[] this->flattened_output[i][0];
    ...
    for(int i = 0; i < threadcnt; i++)
    {
        ...
        this->flattened_output[i] = new Matrix* [this->map_count];
        this->output_derivative[i] = new Matrix* [1];
        this->layers_delta[i] = new Matrix* [1];
        this->layers_delta_helper[i] = new Matrix* [1];
