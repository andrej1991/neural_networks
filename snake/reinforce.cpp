...
void reinforcement_snake(Network &net, StochasticGradientDescent &learn, double learning_rate, double regularization_rate, int input_row, int input_col, double momentum, double denominator)
{
    ...
    //g.play();
    //return;
    MNIST_data *d, *d2, *train_incaseof_fail;
    Matrix action;
    ...
    while(!g.quit)
    {
        ...
        while(SDL_PollEvent(&event) != 0)
        {
            if(event.type == SDL_QUIT)
            {
                ...
                cout << "szóras: " << szoras << endl;
                cout << g.score << endl;
            }
        }
        d = new MNIST_data(input_row, input_col, 4, 3);
        //d2 = new MNIST_data(input_row, input_col, 4, 3);
        ...
        if(!gameover)
        {
            ...
            for(int i = 0; i < 3; i++)
            {
                for(int j=0; j<100;j++)
                {
                    for(int k=0; k<100;k++)
                    {
                        if(d[0].input[i]->data[j][k] != d2[0].input[i]->data[j][k])
                        {
                            cerr << d[0].input[i]->data[j][k] << endl;
                            cerr << d2[0].input[i]->data[j][k] << endl;
                            cerr << "fuck " << i << "  " << j << "  " << k << endl;
                            throw exception();
                        }
                    }
                }
